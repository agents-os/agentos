...
from agentos.agent.agent_planner import ask_llm, generate_plan, execute_step
...
def main(query: str, max_steps: int = 10):
    ...
    try:
        ...
        with console.status("[bold blue]Analyzing task and generating goal...", spinner="dots"):
            goal = ask_llm(
        ...
        if len(plan) > max_steps:
            console.print(f"[yellow]⚠️  Plan has {len(plan)} steps, limiting to {max_steps} for safety[/yellow]")
            plan = plan[:max_steps]
            
        logger.info(f"Generated plan with {len(plan)} steps")
        
        plan_table = Table(title="🗺️ Execution Plan", show_header=True, header_style="bold yellow")
        ...
        with Progress(
            ...
            for i, step in enumerate(plan, 1):
                try:
                    progress.update(main_task, description=f"Step {i}/{len(plan)}: {step[:50]}...")
                    ...
                    console.print(f"[dim]🧠 Reasoning:[/dim] [italic]{explanation}[/italic]")
                    console.print(f"[bold green]⚙️  Command:[/bold green] [code]{command}[/code]")
                    ...
                    if output:
                        ...
                        status_emoji = "✅" if return_code == 0 else "❌"
                        console.print(f"[dim]{status_emoji} Command completed (exit code: {return_code})[/dim]")
                    ...
                    if return_code != 0:
                        console.print(f"[yellow]⚠️  Step {i} completed with warnings (exit code: {return_code})[/yellow]")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        main("create a python file that takes excel file and prints each cell content in column B into the terminal")
