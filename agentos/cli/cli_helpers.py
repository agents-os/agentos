...
from agentos.core import config, utils
...
def load_manifest(path: str) -> dict:
    ...
    try:
        with Progress(
            ...
            configs = config.yaml_to_json(path)
        ...
        if configs["model_provider"].lower() not in valid_providers:
            console.print(f"[red]❌ Invalid model provider: {configs['model_provider']}[/red]")
        ...
        console.print(f"[green]✓[/green] Manifest loaded: [cyan]{configs['name']}[/cyan]")
...
def show_welcome():
    ...
    table = Table(title="⚡ Quick Start", border_style="green", show_header=True, header_style="bold green")
    table.add_column("Command", style="cyan", no_wrap=True)
    table.add_column("Description", style="white")
    
    table.add_row("agentos run <manifest> --task '<task>'", "🚀 Run an agent with a task")
    ...
    console.print()
    console.print("[dim]💡 Tip: Use [cyan]--help[/cyan] with any command for more options[/dim]\n")
